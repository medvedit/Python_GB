...
# Синтаксис:
            # str.count(sub[, start[, end]]) -> int (возращяет цнлое число, колличество вхождений в искомой строке)
# Параметры:
...
print('колличество букв "и" ровно int',x1) # колличество букв "и" ровно int 5
print(type(x1)) # <class 'int'>
...
print(x.encode('ascii', errors='ignore'))   # Тут мы тоже утверждаем, что в значениях строки x нет русских букв,
                                            ...
                                            # то errors='ignore', т.е. пропустим ошибку, и пойдем проверять тексть до последнего символа.
# Ответ:
        # b'   '

print(x.encode('ascii', errors='replace'))  # Здесь подобная утвеждение и проверка,
                                            # за исключением errors='replace'.
                                            # В этом решении вместо каждого ощибочного символа появится знак ? .
# ответ:
        # b'???????? ? ?????? ??????'
...
for item in x:
    item = item.replace(' ', '\t') # встовляет в строку вместо пробелов знак табуляции
    s = item.expandtabs(13) # вместо знака табуляции пропуск в печати последующего слова,
                            # от перврго знака текущего слова,
...
# Описание:
            ...
            # Буквенные символы - это символы, определенные в базе данных символов Юникода как “буква”.
...
# Описание:
            ...
            # Используйте keyword.iskeyword(), чтобы проверить, sявляется ли строка зарезервированным
...
# Описание:
            ...
            # Непечатаемые символы - это символы, определенные в базе данных символов Юникода как "другие"
...
# Это происходит потому, что строка str - это неизменяемый тип и при конкатации (сложении строк)
...
string_end = string.translate(tbl) # метод .translate(....) описпн ниже, предпоследнее описание.
...
# ____________________________________________________________ 'removesuffix' (удаляет суфикс) _________________________________________________________________
...
titlecase(x) # "They're Bill's Friends From The Uk"
...
# Описание:
            ...
            # последовательность. При индексировании по порядковому номеру таблицы Юникода (целому числу),
            # объект table может выполнять любое из следующих действий:
            # возвращать порядковый номер Юникода или строку,
...
# Описание:
            ...
            # если str содержит несвязанные символы или если категория Юникода результирующего символа не
...
# print("-42".zfill()) - ошибка, нет параметра в функии .zfill(_?_)
# Traceback (most recent call last):
#   File "<stdin>", line 1, in <module>
