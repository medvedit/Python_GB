...
def moon_day():
    now_data = datetime.datetime.today() # текущая дата и время.
    ...
    lunar_day = round(difference_day % 29.530588853, 1) # остаток от деления равен - Лунный день. ))))

    return lunar_day

moon = moon_day() # Понимаю, что делаю не красиво, но мне внутри этого файла требуется лунный день. Пришлось так сделать((


def moon_age(day):  # функция вывода возраста луны в бот.
    if day >= 0 and day <= 14.75: # 14.75 - это половина цикла луны. До этого значения рос, далее убывание.
...
def moon_phase(day): # Функция вывода фазы луны относительно лунного дня.
...
def percent_moon(m_day): # Функция вывода возраста луны в процентах от полной.
    if m_day >= 0 and m_day <= 14.75: # относительно полнолуния (14.75) процент видимости луны растет или уменьшается.
        return f'Возраст луны (% от полной) = {round(height_moon(moon), 1)}%' + emoji.emojize('📈')
    else:
        return f'Возраст луны (% от полной) = {round(fall_moon(moon), 1)}%' +  emoji.emojize('📉')


def height_moon(a): # функция процента видимости луны, при росте луны.
    b = float(14.75)
    c2 = (b - a) / b * 100
    return 100 - (c2)


def fall_moon(a): # функция процента видимости луны, после полнолуния.
    b = float(14.75)
    c2 = (a - b) / b * 100
    return 100 - (c2)
